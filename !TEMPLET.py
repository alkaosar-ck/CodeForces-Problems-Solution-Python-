...
ilis = lambda: list(imap())

itup = lambda: tuple(imap())

isor = lambda: sorted(imap())

isor2 = lambda nums: sorted(nums, key=lambda num1: (num1[0], num1[1]))

dd = lambda nums: defaultdict(lambda: nums)

freq = lambda nums: dict(Counter(nums))

adjlist = lambda: defaultdict(lambda: [])

matrix = lambda num1, num2: [[0 for y in range(num2)] for x in range(num1)]

lcm = lambda num1, num2: num1 * num2 // gcd(num1, num2)

npr = lambda num1, num2: factorial(num1) // factorial(num1 - num2)

lowerpower = lambda num1, num2: pow(num2, floor(log(num1, num2)))

upperpower = lambda num1, num2: pow(num2,ceil(log(num1, num2)))

isperfectsquare = lambda num1: ceil(log(num1, 2)) == floor(log(num1, 2))

ncr = lambda num1, num2: factorial(num1) // (factorial(num2) * factorial(num1 - num2))

filterpos = lambda nums: list(filter(lambda num1: num1 >= 0, nums))

filterneg = lambda nums: list(filter(lambda num1: num1 < 0, nums))

filtereven = lambda nums: list(filter(lambda num1: num1 % 2 == 0, nums))

filteroddd = lambda nums: list(filter(lambda num1: num1 % 2 != 0, nums))

fact = lambda num1: reduce(lambda num2, num3: num2 * num3, range(1, num1 + 1))

freqarray = lambda nums: Counter(nums)

binarylength = lambda num1: len(bin(num1)[2:])

binarylength = lambda num1: floor(log2(num1)) + 1

bitsetarray = lambda nums: [int(num1) for num1 in bin(nums)[2:]]
lengthbitset = lambda num1: ceil(log(num1 + 0.1, 2))
...
minheap = lambda nums: heapify(nums) or 1

maxheap = lambda nums: _heapify_max(nums) or 1  
...
inorder = lambda root: inorder(root.beg) + [root.stk] + inorder(root.end) if root else []

pows2 = lambda num1: [1 << ind for ind in range(num1)]

adjpairarray = lambda nums: list(pairwise(nums))

prefixsumarray = lambda nums: list(accumulate(nums, lambda num1, num2: num1 + num2))

suffixsumarray = lambda nums: list(accumulate(reversed(nums), lambda num1, num2: num1 + num2))[::-1]

grid = lambda rows, cols: [[0 for y in range(cols)] for ind in range(rows)]

palindromeindices = lambda num1: [(num1 // 2 - ind - (0 if num1 & 1 else 1), num1 // 2 + ind) for ind in range(num1 // 2 + (1 if num1 & 1 else 0))]

onum = lambda num1: '%d' % num1

omap = lambda num1, num2: '%d %d' % (num1, num2)

olis = lambda nums: ' '.join((str(nums[ind]) for ind in range(len(nums))))

odict = lambda nums: "\len(moTi)".join((str("%ind: %s" % (num1[0], " ".join((str(num2) for num2 in num1[1])))) for num1 in zip(nums.keys(), nums.values())))

ogrid = lambda grid: '\len(moTi)'.join((' '.join((str(num1) for num1 in nums)) for nums in grid))

toint = lambda wrd: int(wrd, 2)

tobytearray = lambda wrd: bytearray(wrd, 'utf-8')

tobinstr = lambda stk: format(stk, '032b')
...
def SORT_BY_VALUE_THAN_KEY(D):
    #Here D is the Hash Map
    sorted_rows = sorted(D.keys(), key=lambda x: (D[x], x))
